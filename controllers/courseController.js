...
export const getCourseById = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      error: "An error occurred while fetching the course. Server error.",
    });
  }
};
...
export const createCourse = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      error: "An error occurred while creating the course. Server error.",
    });
  }
};
...
export const deleteCourse = async (req, res) => {
  try {
    ...
    // remove uplod file from the local upload
    try {
    ...
    return res.status(500).json({
      success: false,
      error: "An error occurred while deleting the course. Server error.",
    });
  }
};
  ...
  try {
    ...
    if (!userId) {
      return res.status(401).json({
        success: false,
        message: "Unauthorized. Please log in to rate the course.",
      });
    }
    ...
    if (!Number.isFinite(rating) || rating < 1 || rating > 5) {
      return res.status(400).json({
        success: false,
        message:
          "Invalid rating value. Rating should be a number between 1 and 5.",
      });
    }
...
export const getMyRating = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res.status(401).json({
        success: false,
        message: "Unauthorized. Please log in to view your rating.",
      });
